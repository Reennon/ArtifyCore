    ...
    internal class ModuleDispatcher : ILinkerBaseFields
    {
        ...
        internal class Body : IModuleBody
        {
            ...
            { 
                ...
                {
                    pathToLanguages = Convert.ToString(o["Environments"]).Replace(@"\", "/");
                    pathToModules = Convert.ToString(o["Modules"]).Replace(@"\", "/");
                }
                ...
                {
                    result = Path.GetFileName(file);
                    string[] fileEntries3=null;
                    if (RuntimeInformation.IsOSPlatform(OSPlatform.Windows))
                    {
                        fileEntries3 =
                            Directory.GetFiles(_body.pathToModules + "\\" +
                                               result); //all file in directory
                    }
                    if (RuntimeInformation.IsOSPlatform(OSPlatform.Linux))
                    {
                        fileEntries3 =
          ...
          RegexOptions.Compiled | RegexOptions.IgnoreCase);
                ...
                if (RuntimeInformation.IsOSPlatform(OSPlatform.Linux))
                {
                    ...
                    DirectoryInfo parentDir = Directory.GetParent(ExecutableLanguage["python.exe"]);
                    var myParentDir = parentDir.FullName;
                    parentDir = Directory.GetParent(myParentDir);
                    myParentDir = parentDir.FullName;
                    myParentDir += "/Linux/" + myenv + "/bin";
                    string command = $@"cd {myParentDir} ; "+ "source activate ; " + var1 + "deactivate";
            ...
            )
            {
                ...
                if (RuntimeInformation.IsOSPlatform(OSPlatform.Linux))
                {
                    ...
                    proc.StartInfo.RedirectStandardOutput = true;
                    proc.Start();
                    //python enemy.py "{\"NumberOfImages\":2,\"Image\":[\"/home/yura/Downloads/JetBrains.Rider-2020.3.2/ArtifyCore-ModuleDispatcher/images/image1.png\",\"D:/ProjArtify/ArtifyCore/Users/Yura/image2.png\"],\"PathToModule\":\"enemy\"}"
            ...
            {
                // Open the named pipe.
                var server = new NamedPipeServerStream("NPtes.pipe");
